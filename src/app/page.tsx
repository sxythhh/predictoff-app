...
function HeroEmbed() {
  ...
  useEffect(() => {
    ...
    // Intercept wheel on the container and forward to the scroll parent
    const container = containerRef.current;
    const onWheel = (e: WheelEvent) => {
      const scrollParent = container?.closest("[class*='overflow-y-auto']") as HTMLElement | null;
      if (scrollParent) scrollParent.scrollTop += e.deltaY;
    };
  ...
  return (
    <div ref={containerRef} className="relative w-full overflow-hidden" style={{ height: "clamp(400px, 50vh, 600px)" }}>
      <iframe
        ref={iframeRef}
        src="https://friendly-popover-009361.framer.app/"
    ...
    <nav className="fixed bottom-0 left-0 right-0 z-40 lg:hidden safe-area-pb">
      <div className="relative flex items-end h-[44px] bg-bg-card dark:bg-bg-surface">
        ...
        <div className="relative flex flex-col items-center" style={{ width: 64 }}>
          ...
          {/* Circle button */}
          <button
            onClick={() => { onBetslipOpen(); window.dispatchEvent(new Event("open-betslip")); }}
        ...
        {/* Compete */}
        <NavBtn label="Compete" active={false} onClick={() => { window.location.href = "/tournaments"; }}>
