...
function EventFilter() {
  ...
  return (
    <div className="flex items-center gap-1.5 px-2 pt-3 pb-1">
      <button
        type="button"
        onClick={() => { changeLive(false); setShowFavourites(false); }}
      ...
      <button
        type="button"
        onClick={() => { changeLive(true); setShowFavourites(false); }}
        className={`h-8 px-3.5 rounded-full text-[13px] font-medium transition-colors flex items-center gap-1.5 ${
          isLive && !showFavourites
...
function MobileNav({
  ...
  function NavBtn({ label, active, color, onClick, children }: {
    label: string; active: boolean; color?: string; onClick: () => void; children: React.ReactNode;
  }) {
    return (
      <button onClick={onClick} className={`flex-1 h-14 flex flex-col items-center justify-center gap-0.5 transition-colors cursor-pointer ${active ? (color ?? "text-text-primary") : "text-text-muted"}`}>
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 lg:hidden bg-bg-page border-t border-border-primary safe-area-pb">
      <div className="flex items-stretch">
        {/* Sports */}
        <NavBtn label="Sports" active={activeTab === "sports" && !isLive} onClick={() => { onTabChange("sports"); if (isLive) changeLive(false); }}>
          <svg width="18" height="18" viewBox="0 0 20 20" fill="none">
            <circle cx="10" cy="10" r="7.5" stroke="currentColor" strokeWidth="1.5"/>
            <path d="M10 2.5V17.5M2.5 10H17.5M4 5L16 15M16 5L4 15" stroke="currentColor" strokeWidth="1" opacity="0.4"/>
          </svg>
        </NavBtn>

        {/* Live */}
        <NavBtn label="Live" active={isLive} color={isLive ? "text-status-live" : undefined} onClick={() => changeLive(!isLive)}>
          <LiveIcon />
        </NavBtn>

        {/* Betslip — center button */}
        <div className="flex-1 flex items-center justify-center">
          <button
            onClick={onBetslipOpen}
            className="relative w-12 h-12 -mt-5 rounded-full flex items-center justify-center bg-accent shadow-[0_2px_12px_rgba(37,182,98,0.3)] active:scale-95 transition-transform cursor-pointer"
          >
            <svg width="18" height="18" viewBox="0 0 20 20" fill="none">
            ...
            {betslipCount > 0 && (
              <span className="absolute -top-1 -right-1 min-w-[18px] h-[18px] flex items-center justify-center rounded-full bg-white text-accent text-[10px] font-bold px-1">
                {betslipCount}
              </span>
            )}
          </button>
...
function MobileNavWithCount({ activeTab, onTabChange, onBetslipOpen }: {
  activeTab: string;
  onTabChange: (tab: "sports" | "live" | "betslip" | "bets" | "social") => void;
  onBetslipOpen: () => void;
}) {
  const { items } = useBaseBetslip();
  return <MobileNav activeTab={activeTab} onTabChange={onTabChange} onBetslipOpen={onBetslipOpen} betslipCount={items.length} />;
}
